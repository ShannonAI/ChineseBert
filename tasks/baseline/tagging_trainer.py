...
from datasets.bert_weibo_ner_dataset import BertWeiboNERDataset
from datasets.bert_ontonotes_ner_dataset import BertOntoNotesNERDataset
...
class BertForTaggingTask(pl.LightningModule):
    def __init__(self, args: argparse.Namespace):
        """initialize model and model-config"""
        super().__init__()
        self.args = args
        if isinstance(args, argparse.Namespace):
            self.save_hyperparameters(args)

        if self.args.task.lower() == "ontonotes4":
            self.entity_labels = BertOntoNotesNERDataset.get_labels()
        elif self.args.task.lower() == "weibo":
            self.entity_labels = BertWeiboNERDataset.get_labels()
        else:
            raise ValueError
        ...
        self.bert_config = BertConfig.from_pretrained(self.bert_dir, output_hidden_states=False,
                                                      num_labels=self.num_labels, hidden_dropout_prob=self.args.hidden_dropout_prob)
    ...
    def compute_loss(self, logits, labels, loss_mask=None):
        """
        Args:
            logits: FloatTensor, shape of [batch_size, seq_len, num_labels]
            labels: LongTensor, shape of [batch_size, seq_len]
            loss_mask: LongTensor, shape of [batch_size, seq_len]
        """
    ...
    def training_step(self, batch, batch_idx):
        ...
        sequence_logits = self.forward(input_ids, attention_mask, token_type_ids)[0]
        loss = self.compute_loss(sequence_logits, labels, loss_mask=attention_mask)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        confusion_matrix = self.ner_evaluation_metric(argmax_labels, labels, sequence_mask=attention_mask)
    ...
    def validation_epoch_end(self, outputs):
        ...
        return {"val_loss": avg_loss, "val_log": tensorboard_logs,
    ...
    def _load_dataset(self, prefix="test"):
        if self.args.task.lower() == "ontonotes4":
            dataset = BertOntoNotesNERDataset(self.args.data_dir, prefix=prefix,
            ...
            dataset = BertWeiboNERDataset(self.args.data_dir, prefix=prefix,
                                      vocab_file=os.path.join(self.args.bert_path, "vocab.txt"),
                                      max_length=self.args.max_length,)
        else:
            raise ValueError("ERROR: TASK name does not in [weibo, ontonotes4]")
    ...
    def get_dataloader(self, prefix="train", limit=None) -> DataLoader:
        ...
        dataloader = DataLoader(dataset=dataset, sampler=data_sampler, batch_size=batch_size,
                                num_workers=self.args.workers, collate_fn=partial(collate_to_max_length, fill_values=[0, 0, 0, 0]),
                                drop_last=False)
...
def get_parser():
    ...
    parser.add_argument("--task", choices=["ontonotes4", "weibo"], default="ontonotes4", type=str, help="task sign")
...
def evaluate():
    ...
    model = BertForTaggingTask.load_from_checkpoint(checkpoint_path=args.checkpoint_path,
